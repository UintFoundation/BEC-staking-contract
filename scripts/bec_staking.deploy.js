// bec_staking.ride deploy script. To run execute `surfboard run path/to/script`

// wrap out script with async function to use fancy async/await syntax
(async () => {
    // Functions, available in tests, also available here
    const script = compile(file('bec_staking.ride'));
    ...
    if (dappSeed == null){
        throw new Error(`Please provide dappSeed`)
    }
